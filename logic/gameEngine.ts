import { Player, Team, GameMap, MatchResult, Role, StrategyType, Rivalry } from '../src/types';
import { TAG_PREFIXES, TAG_SUFFIXES, LEGEND_TAGS, ROLES, STRATEGIES, TOTAL_SEASON_WEEKS } from '../constants';

export const createPlayer = (tier: 'Normal' | 'Legend' = 'Normal'): Player => {
  const id = Math.random().toString(36).substr(2, 9);
  const role = ROLES[Math.floor(Math.random() * ROLES.length)];
  const age = Math.floor(Math.random() * 8) + 16;
  
  let gamertag, aim, iq, potential;
